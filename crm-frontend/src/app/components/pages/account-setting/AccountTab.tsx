...
const AccountTab = () => {
  ...
  const profileMutation = useMutation({
    ...
    onError: (err: any) => {
      setError('Не удалось обновить профиль: ' + (err.message || 'Неизвестная ошибка'))
  ...
  const avatarMutation = useMutation({
    ...
    onError: (err: any) => {
      setError('Не удалось загрузить аватар: ' + (err.message || 'Неизвестная ошибка'))
            ...
            <Box textAlign="center" display="flex" justifyContent="center">
              <Box>
                <Avatar
                  src={
                    avatarPreview ||
                    (profile.avatar ? `http://localhost:4200${profile.avatar}` : '/images/profile/user-1.jpg')
                  }
                  alt="user"
